var background = (function() {
  ...
  var render = function() {
    ...
    if (state.get().control.background.image.active) {
      helper.removeClass(background, "is-hidden");
      html.style.setProperty("--background-image", "url(\"" + state.get().control.background.image.url + "\")");
      html.style.setProperty("--background-blur", state.get().control.background.image.blur + "px");
      html.style.setProperty("--background-grayscale", state.get().control.background.image.grayscale);
      html.style.setProperty("--background-opacity", state.get().control.background.image.opacity);
      html.style.setProperty("--background-accent-opacity", state.get().control.background.image.accentOpacity);
      if (state.get().control.background.image.blur > 0 && state.get().control.background.image.grayscale > 0) {
        backgroundImage.style.setProperty("filter", "blur(var(--background-blur)) grayscale(var(--background-grayscale))");
      } else if (state.get().control.background.image.blur > 0 && state.get().control.background.image.grayscale == 0) {
